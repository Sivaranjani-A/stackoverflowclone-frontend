  ...
  return (
    ...
    <nav className="navbar navbar-expand-lg navbar-light bg-light navbar-fixed-top ml-2 mr-2" style={{ backgroundColor: "#e3f2fd !important" }} >
      <div className='col-lg-4 ml-2'>
        <button className="btn navbar-brand col px-md-4" onClick={() => Home()}><FontAwesomeIcon icon={faHome} color={"orange"} />Stack <b>overflow</b></button>
        ...
        <form onSubmit={formik.handleSubmit}>
          <div className='col d-flex justify-content-center'>
            <input
              ...
              aria-label="Search" />
            <button className="btn btn-outline-success my-2 my-sm-0 m-2" type={"submit"}><FontAwesomeIcon icon={faSearch} /></button>
          </div>
        </form>
      </div>
      ...
      <div className='col-lg-4 '>
        <div className="collapse navbar-collapse d-flex justify-content-end" id="navbarNavAltMarkup">
          <div className="navbar-nav">
            ...
            <Link to="/Portal/Profile" className="btn btn-info m-2">View Profile</Link>
            <button className="btn btn-outline-danger m-2" onClick={() => { doLogout() }}> <FontAwesomeIcon icon={faSignOut} /> Logout </button>
          </div>
        </div>
      </div>
    </nav>
